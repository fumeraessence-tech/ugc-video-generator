...
export async function POST(request: NextRequest) {
  ...
  if (!body?.jobId || !body?.sceneNumber) {
    return NextResponse.json(
      { error: "jobId and sceneNumber are required" },
      { status: 400 }
    );
  }
  ...
  try {
    const res = await fetch(`${backendUrl}/api/v1/storyboard/regenerate-scene`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        job_id: body.jobId,
        scene_number: body.sceneNumber,
        updated_script: body.updatedScript || null,
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: err.detail ?? "Scene regeneration failed" },
        { status: res.status }
      );
    }
